...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				...
				int goal_title = goalState.getTile(i, j);
				if (title != 0) {
					if (title != goal_title) {
						output++;
	...
	public int computeH2(Node currentState) {
		...
		/* Enter your code here */
		for (int i = 1; i < 9; i++) {
			int[] position = currentState.getLocation(i);
			int[] goal_pos = goalState.getLocation(i);
			result += PuzzleUtils.manhattanDistance(position, goal_pos);
		}
		return result;
	}
